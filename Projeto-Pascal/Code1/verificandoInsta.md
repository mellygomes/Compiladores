...
void ObterLexema(AnalisadorLexico *lex) {
    FILE *arquivoSaid;
    arquivoSaid = fopen("tblLexema.txt", "a");
...
void estadoInicial(AnalisadorLexico *lex) {
    ...
    switch (ObterCharactere(lex)) {
        ...
        case 'p':
            ...
            printf("%i", ObterPosicaoDaCabeca(lex));

            strToLower(lex->lexema);
            if (strcmp(lex->lexema, "program") == 0) {
                ObterLexema(lex);
            } else {
                lex->cabeca = lex->cabeca - 6;
                printf("nao comparou");
            }
